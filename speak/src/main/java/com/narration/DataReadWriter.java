...
public class DataReadWriter extends DataConstants{
    ...
    private static ArrayList<Exercise> getExercises(JSONArray exercisesJSON){
        ...
        for (int i = 0; i <exercisesJSON.size(); i++){
            ...
            ArrayList<String> options = getOptions((JSONArray)exerciseJSON.get(OPTIONS));
            String answer = (String)exerciseJSON.get(ANSWER);

            if (type == "Matching")
                exercises.add(new Matching(question, options, answer));
            if (type == "FillIn")
                exercises.add(new FillIn(question, options, answer));
            if (type == "Conversation")
                exercises.add(new Conversation(question, options, answer));
        }

        return exercises;
    }

    private static ArrayList<String> getOptions(JSONArray jsonArray) {
        //  TODO
    }
